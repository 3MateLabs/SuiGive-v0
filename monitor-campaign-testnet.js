// Script to continuously monitor a campaign object on testnet
...
if (!CAMPAIGN_ID) {
  ...
  console.error("Usage: node monitor-campaign-testnet.js <campaign-id>");
  process.exit(1);
}
  ...
  try {
    ...
    if (!previousState) {
      ...
      if (previousState.raisedSgUSD !== currentState.raisedSgUSD) {
        ...
        const beforeAmount = BigInt(previousState.raisedSgUSD || '0');
        const afterAmount = BigInt(currentState.raisedSgUSD || '0');
